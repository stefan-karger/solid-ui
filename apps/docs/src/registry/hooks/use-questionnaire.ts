import { createContext, createEffect, createMemo, createSignal, createUniqueId, on, onCleanup, onMount, untrack, useContext, type ComponentProps, type JSX } from "solid-js";
  ...
  for (const definition of collection.items) {
    ...
    if (!registration) {
      if (!definition.disabled) {
        warnings.push(
          `Item "${definition.name}" is defined but has no rendered QuestionnaireItem.`,
        );
      }
...
function createQuestionnaireRoot(props: CreateQuestionnaireRootParameters) {
  ...
  (() => createQuestionnaireCollection(props.items));
