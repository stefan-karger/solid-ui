...
type ResizablePanelGroupProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T, RootProps<T>> & Pick<ComponentProps<T>, "class">;
...
type ResizableHandleProps<T extends ValidComponent = "button"> =
  PolymorphicProps<T, HandleProps<T>> &
...
const ResizableHandle = <T extends ValidComponent = "button">(
  ...
  return (
    <Handle
      class={cn(
        "relative cn-resizable-handle flex w-px items-center justify-center bg-border after:absolute after:inset-y-0 after:left-1/2 after:w-1 after:-translate-x-1/2 focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1 data-[orientation=vertical]:h-px data-[orientation=vertical]:w-full data-[orientation=vertical]:after:left-0 data-[orientation=vertical]:after:h-1 data-[orientation=vertical]:after:w-full data-[orientation=vertical]:after:translate-x-0 data-[orientation=vertical]:after:-translate-y-1/2 [&[data-orientation=vertical]>div]:rotate-90",
        local.class,
      )}
    ...
    >
      <Show when={local.withHandle}>
        <div class="z-10 cn-resizable-handle-icon flex shrink-0" />
      </Show>
    </Handle>
  );
};
