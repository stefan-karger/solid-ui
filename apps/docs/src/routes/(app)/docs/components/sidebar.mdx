...
For the full example layout (SolidStart paths):
...
<Callout type="note">
  `solidui-cli init` scaffolds **Tailwind CSS v3**. The sidebar component uses `w-[var(--sidebar-width)]` and other v3-compatible arbitrary values. The solid-ui.com docs app runs Tailwind v4 separately.
</Callout>
