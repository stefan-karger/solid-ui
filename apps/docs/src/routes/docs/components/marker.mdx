...
The `Marker` component displays inline conversation markers such as status updates, system notes, bordered rows, and labeled separators. Compose it with [`Message`](/docs/components/kobalte/message) in a conversation thread.
...
Set `role="status"` and include a [`Spinner`](/docs/components/kobalte/spinner) for streaming or
