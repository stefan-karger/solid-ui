...
async function captureScreenshots() {
  ...
  const blocks = blockIds.filter((block) => {
    ...
    const lightPath = path.join(REGISTRY_PATH, `styles/v1/${block}-light.png`)
    const darkPath = path.join(REGISTRY_PATH, `styles/v1/${block}-dark.png`)
  ...
  for (const block of blocks) {
    const pageUrl = `http://localhost:4000/view/v1/${block}`
