...
export const Index: Record<string, Record<string, any>> = {`
  ...
  for (const style of styles) {
    ...
    const { registry: importedRegistry } = await import(`../registry/${style.name}/registry.ts`)
  ...
  "${style.name}": {`
    ...
    "${item.name}": {
      ...
      files: [${files.map((file) => {
        const filePath = `src/registry/${style.name}/${file.path}`
        return `{
        path: "${filePath}",
        type: "${file.type}",
        target: "${file.target ?? ""}"
...
async function buildRegistryJsonFile(styleName: string) {
  ...
  const { registry: importedRegistry } = await import(`../registry/${styleName}/registry.ts`)
  ...
  const fixedRegistry = {
    ...
    items: registry.items.map((item) => {
      const files = item.files?.map((file) => {
        return {
          ...file,
          path: `src/registry/${styleName}/${file.path}`
        }
      })
  ...
  const tempRegistryPath = path.join(process.cwd(), `src/registry-${styleName}.json`)
...
async function buildRegistry(styleName: string) {
  return new Promise((resolve, reject) => {
    ...
    exec(`pnpm dlx shadcn build src/registry-${styleName}.json --output ${outputPath}`, (error) => {
      if (error) {
        reject(error)
      } else {
        resolve(undefined)
...
try {
  ...
  for (const style of styles) {
    if (existsSync(path.join(process.cwd(), `src/registry-${style.name}.json`))) {
      await fs.unlink(path.join(process.cwd(), `src/registry-${style.name}.json`))
    }
  }
